 ...
 * @ Modified time: 2021-08-01 20:47:18
...
const getTebakKata = (chatId) => new Promise((resolve, reject) => {
	try {
		axios.get(`https://neoxr-api.herokuapp.com/api/games/whathis?apikey=yntkts`)
			.then((res) => {
				let ans = res.data.data.jawaban
				saveRoom(chatId, ans)
...
const getTebakSiapa = (chatId) => new Promise((resolve, reject) => {
	try {
		axios.get(`https://api.zeks.me/api/siapa-aku?apikey=apivinz`)
			.then((res) => {
				let ans = res.data.data.jawaban
				saveRoom(chatId, ans)
				resolve(res.data)
...
const getTebakLontong = (chatId) => new Promise((resolve, reject) => {
	try {
		axios.get(`https://api.zeks.me/api/cak-lontong?apikey=apivinz`)
			.then((res) => {
				let ans = res.data.data.jawaban
				let ket = res.data.data.keterangan
				saveRoom(chatId, ans, ket)
				resolve(res.data)
			})
	} catch (e) {
		reject(e)

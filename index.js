...
const start = async (client = new Client()) => {
    try {
        ...
        try {
            ...
            // await get(`https://api.myquran.com/v1/sholat/jadwal/1102/${tgl}`)
            ...
            client.getAllGroups().then((groups) => {
                ...
                if (groups.length > groupLimit) {
                    ...
                    client.simulateTyping(chat.groupMetadata.id, true).then(async () => {
                        client.sendText(chat.groupMetadata.id, `Hai guys 👋 perkenalkan saya SANBOT. Untuk melihat perintah atau menu yang tersedia pada bot, kirim *${prefix}menu*. Tapi sebelumnya pahami dulu *${prefix}tnc*`)
                    })
                }
            })
        })
            ...
            try {
                ...
                if (message.author != host && isAntiDelete) {
                    ...
                    if (message.type == 'sticker') {
                        const mediaData = await decryptMedia(message)
                        await client.sendImageAsSticker(message.from, mediaData, { pack: 'Anti delete by', author: 'SANBOT', keepScale: true })
                    }
                }
            } catch (err) {

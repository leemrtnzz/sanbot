...
import { getLocationData, cariKasar, urlShortener, schedule, canvas, cekResi, tebak, scraper, menuId, sewa, list, note, api } from './lib/index.js'
...
const HandleMsg = async (message, browser, client = new Client()) => {
    ...
    const resMsg = {
        ...
        success: {
            ...
            greeting: `Hai guys 👋 perkenalkan saya SANBOT 🤖` +
    ...
    try {
        ...
        const stickerMetadata = { pack: 'Created with', author: 'SANBOT', keepScale: true }
        const stickerMetadataCircle = { pack: 'Created with', author: 'SANBOT', circle: true }
        const stickerMetadataCrop = { pack: 'Created with', author: 'SANBOT', cropPosition: 'center' }
        // Bot Prefix Aliases
        const regex = /^[\\/!$^%&+.,-](?=\w+)/
        ...
        const startTebakRoomTimer = (seconds, answer) => {
            const hint = answer.replace(/\s/g, '\t').replace(/[^aeiou\t]/gi, '?')
        ...
        const startTebakRoomTimerLontong = (seconds, answer, keterangan) => {
            const hint = answer.replace(/\s/g, '\t').replace(/[^aeiou\t]/gi, '?')
            sleep(seconds * 1000 / 4).then(async () => {
                const ans = await tebak.getAns(from)
                if (ans === false) return true
        ...
        switch (true) {
            case /^\b(hi|hy|halo|hallo|hai|hei|hello)\b/i.test(chats): {
                await reply(`Halo ${pushname} 👋`)
                break
            }
            case /^\b(terimakasih|terima kasih|terimakasi|terima kasi|tq|thanks|thank|thank you|thanks you|thank u|thanku|thanksyou|thankyou)\b/i.test(chats): {
                await reply(`Sama sama kak`)
                break
            }
            case /^p$/i.test(chats): {
            ...
            case /assalamualaikum|ass'alamualaikum|ass'alamu alaikum|assalamu'alaikum|asalamualaikum|assalamu'alaykum/i.test(chats): {
            ...
            case /\b(bot|san|SANBOT)\b/ig.test(chats): {
                if (!isCmd) {
                    let txt = chats.replace(/@\d+/g, '')
                    // return doSimi(txt)
                    return reply(`Iya kak, aku disini`)
                }
                break
            }
            case /^>/.test(chats): {
        ...
        if (isCmd) {
            ...
            switch (command) {
                ...
                case 'help':
                case 'start': {
                    await sendText(menuId.textList(pushname, t, prefix))
                ...
                case 'minf':
                case 'm-info': {
                    await sendText(menuId.textMinf(pushname, t, prefix))
                    if ((isGroupMsg) && (isGroupAdmin)) sendText(`Menu Admin Grup: *${prefix}menuadmin*`)
                    if (isOwnerBot) sendText(`Menu Owner: *${prefix}menuowner*`)
                    break
                }
                case 'minf2':
                case 'm-info2': {
                    await sendText(menuId.textMinf2(pushname, t, prefix))
                    if ((isGroupMsg) && (isGroupAdmin)) sendText(`Menu Admin Grup: *${prefix}menuadmin*`)
                    if (isOwnerBot) sendText(`Menu Owner: *${prefix}menuowner*`)
                    break
                }
                case 'mcon':
                case 'm-convert': {
                    await sendText(menuId.textMcon(pushname, t, prefix))
                    if ((isGroupMsg) && (isGroupAdmin)) sendText(`Menu Admin Grup: *${prefix}menuadmin*`)
                    if (isOwnerBot) sendText(`Menu Owner: *${prefix}menuowner*`)
                    break
                }
                case 'mker':
                case 'm-maker': {
                    await sendText(menuId.textMker(pushname, t, prefix))
                    if ((isGroupMsg) && (isGroupAdmin)) sendText(`Menu Admin Grup: *${prefix}menuadmin*`)
                    if (isOwnerBot) sendText(`Menu Owner: *${prefix}menuowner*`)
                    break
                }
                case 'mdl':
                case 'm-dl': {
                    await sendText(menuId.textMdl(pushname, t, prefix))
                    if ((isGroupMsg) && (isGroupAdmin)) sendText(`Menu Admin Grup: *${prefix}menuadmin*`)
                    if (isOwnerBot) sendText(`Menu Owner: *${prefix}menuowner*`)
                    break
                }
                case 'mac':
                case 'm-audioconv': {
                    await sendText(menuId.textMac(pushname, t, prefix))
                    if ((isGroupMsg) && (isGroupAdmin)) sendText(`Menu Admin Grup: *${prefix}menuadmin*`)
                    if (isOwnerBot) sendText(`Menu Owner: *${prefix}menuowner*`)
                    break
                }
                case 'mslm':
                case 'm-islam': {
                    await sendText(menuId.textMslm(pushname, t, prefix))
                    if ((isGroupMsg) && (isGroupAdmin)) sendText(`Menu Admin Grup: *${prefix}menuadmin*`)
                    if (isOwnerBot) sendText(`Menu Owner: *${prefix}menuowner*`)
                    break
                }
                case 'mran':
                case 'm-random': {
                    await sendText(menuId.textMran(pushname, t, prefix))
                    if ((isGroupMsg) && (isGroupAdmin)) sendText(`Menu Admin Grup: *${prefix}menuadmin*`)
                    if (isOwnerBot) sendText(`Menu Owner: *${prefix}menuowner*`)
                    break
                }
                case 'msrch':
                case 'm-search': {
                    await sendText(menuId.textMsrch(pushname, t, prefix))
                    if ((isGroupMsg) && (isGroupAdmin)) sendText(`Menu Admin Grup: *${prefix}menuadmin*`)
                    if (isOwnerBot) sendText(`Menu Owner: *${prefix}menuowner*`)
                    break
                }
                case 'mhib':
                case 'm-hiburan': {
                    await sendText(menuId.textMhib(pushname, t, prefix))
                    if ((isGroupMsg) && (isGroupAdmin)) sendText(`Menu Admin Grup: *${prefix}menuadmin*`)
                    if (isOwnerBot) sendText(`Menu Owner: *${prefix}menuowner*`)
                    break
                }
                case 'mat':
                case 'm-toxic': {
                    await sendText(menuId.textMat(pushname, t, prefix))
                    if ((isGroupMsg) && (isGroupAdmin)) sendText(`Menu Admin Grup: *${prefix}menuadmin*`)
                    if (isOwnerBot) sendText(`Menu Owner: *${prefix}menuowner*`)
                    break
                }
                case 'mr':
                case 'more': {
                    await sendText(menuId.textMore(pushname, t, prefix))
                    if ((isGroupMsg) && (isGroupAdmin)) sendText(`Menu Admin Grup: *${prefix}menuadmin*`)
                    if (isOwnerBot) sendText(`Menu Owner: *${prefix}menuowner*`)
                    break
                }
                ...
                case 'pml':
                case 'priceml':{
                    await sendText(menuId.textPml(pushname, t, prefix))
                    client.sendContact(from, ownerNumber)
                    .then(() => sendText('Jika ada pertanyaan/membeli silahkan chat nomor di atas ⬆\nChat tidak jelas akan diabaikan.'))
                    break
                }
                case 'menuadmin':{
                    if (!isGroupMsg) return reply(resMsg.error.group)
            ...
            switch (command) {
                ...
                case 'shortlink': {
                    if (args.length == 0) return reply(`ketik ${prefix}sid <url>\nJika error gunakan  ${prefix}short <url>`)
                    if (!isUrl(args[0])) return reply('Maaf, url yang kamu kirim tidak valid. Pastikan menggunakan format http/https')
                    axios.get(`https://api.zeks.me/api/urlshort?apikey=apivinz&url=${args[0]}`)
                    .then(res => {
                        // console.log(res.data.result)
                        console.log(color('[LOGS]', 'grey'),`Shortlink : ${res.data.result} - Asli : ${args[0]}`)
                        reply(`Shortlink : *${res.data.result}*\nAsli : *${args[0]}*`)
                    })
                    break
                }
                case 'short': {
                    ...
                    await reply(`Shortlink : *${shorted}*\nAsli : *${args[0]}*`).catch(e => { return printError(e) })
                    break
                }
                ...
                case 'sid': {
			        ...
			        api.sid(args[0])
			            .then(async(res) => {
                            if (res.status == true){
                                reply(`Shortlink : *${res.short}*\nAsli : *${res.long}*`)
                                console.log(color('[LOGS]', 'grey'),`Shortlink : ${res.short} - Asli : ${res.long}`)
                            } else {
                                reply ('Shortlink error')
                            }
			            })
                        .catch(e => { return printError(e) })
			        break
            ...
            switch (command) {
                ...
                case 'jw': {
                    ...
                    let jadwalMsg = `╔══✪〘 Jadwal Sholat di ${jadwalData.data.lokasi} 〙✪\n`
                    jadwalMsg += `╠> ${jadwal.tanggal}\n`
                    jadwalMsg += `╠> ${jadwal.subuh.substring(3,6)}\n`
                    jadwalMsg += '╚═〘 *SANBOT* 〙'
                    reply(jadwalMsg)
                    }
                    break
            ...
            switch (command) {
                /* #region Primbon */
                case 'artimimpi': {
                    if (args.length == 0) return reply(`Untuk mengetahui arti mimpi \nketik ${prefix}artimimpi (mimpi kamu)\nContoh : ${prefix}artimimpi ular`)
                    api.artimimpi(arg)
                        .then(res => {
                            if (res.status == true) {
                                reply(res.result.string.replace(/\n/g,'\n\n'))
                            } else {
                                reply("Gak nemu bro :(")
                            }
            ...
            switch (command) {
                ...
                case 'tebaksiapa': {
                    const isRoomExist = await tebak.isRoomExist(from)
                    if (isRoomExist) return reply(`Sesi Tebak sedang berlangsung. ${prefix}skip untuk skip sesi.`)
                    await tebak.getTebakSiapa(from).then(async res => {
                        let detik = 60
                        await reply(`Tebak Siapa Aku.\nJawab dengan *membalas pesan ini*.\n\n` +
                            `${q3 + res.data.soal + q3}\n\n` +
                            `Jumlah huruf: ${res.data.jawaban.length}\nWaktunya ${detik} detik.\n*${prefix}skip* untuk skip`)
                            .then(() => {
                                startTebakRoomTimer(detik, res.data.jawaban)
                            })
                    }).catch(e => { return printError(e) })
                    break
                }
                ...
                case 'tebaklontong': {
                    const isRoomExist = await tebak.isRoomExist(from)
                    if (isRoomExist) return reply(`Sesi Tebak sedang berlangsung. ${prefix}skip untuk skip sesi.`)
                    await tebak.getTebakLontong(from).then(async res => {
                        let detik = 60
                        await reply(`Tebak Cak Lontong.\nJawab dengan *membalas pesan ini*.\n\n` +
                            `${q3 + res.data.soal + q3}\n\n` +
                            `Jumlah huruf: ${res.data.jawaban.length}\nWaktunya ${detik} detik.\n*${prefix}skip* untuk skip`)
                            .then(() => {
                                startTebakRoomTimerLontong(detik, res.data.jawaban, res.data.keterangan)
                            })
                    }).catch(e => { return printError(e) })
                    break
                }
            ...
            tebak.getAns(from).then(res => {
                if (res && quotedMsg?.fromMe) {
                    if (res.ans?.toLowerCase() === chats?.toLowerCase()) {
                        ...
                        if (res.data.keterangan.length === 0) {

                        } else {
                            reply(`Keterangan : ${res.data.keterangan}`)
                        }
                        tebak.delRoom(from)
                    } else {
                        reply(`❌ Salah!`)
